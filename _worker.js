...
async function 启动传输管道(WS接口) {
  ...
  WS接口.addEventListener("message", async (事件) => {
    if (!首包数据) {
      首包数据 = true;
      await 解析VL标头(事件.data);
    } else {
      await 传输数据.write(事件.data);
    }
  });
  ...
  function 验证VL的密钥(arr, offset = 0) {
    const uuid = (
      转换密钥格式[arr[offset + 0]] + 转换密钥格式[arr[offset + 1]] + 转换密钥格式[arr[offset + 2]] + 转换密钥格式[arr[offset + 3]] + "-" +
